export const fireballBaseSpeed_anim = 0.5;

export function initializeAllFireballs_anim(fireballs: NodeListOf<HTMLElement>) {
    const fireballs_anim_array: any[] = [];
    fireballs.forEach((fbEl, i) => {
        fireballs_anim_array[i] = {
            x: 0,
            y: 0,
            vx: 0,
            vy: 0,
        ...
        initSingleFireball(fireballs_anim_array[i]);
    });
    return fireballs_anim_array;
}

function initSingleFireball(fb_obj: any) {
    if (!fb_obj.el) return;
    ...
    fb_obj.vx = Math.cos(angle) * fireballBaseSpeed_anim;
    fb_obj.vy = Math.sin(angle) * fireballBaseSpeed_anim;
    fb_obj.el.style.left = fb_obj.x + 'px';
    fb_obj.el.style.top = fb_obj.y + 'px';
}

export function animateFireballs_anim(fireballs_anim_array: any[]) {
    fireballs_anim_array.forEach((fb, i) => {
        if (!fb || !fb.el) return;
        fb.x += fb.vx;
        fb.y += fb.vy;
        ...
        for (let j = i + 1; j < fireballs_anim_array.length; j++) {
            const fb2 = fireballs_anim_array[j];
            if (!fb2 || !fb2.el) return;
            handleFireballCollision_anim(fb, fb2);
        }
    });
    // requestAnimationFrame handled in component
}

function handleWallCollision_anim(fb_obj: any) {
    const viewportWidth = window.innerWidth;
    const viewportHeight = window.innerHeight;
...
function handleFireballCollision_anim(ball1: any, ball2: any) {
...
export async function typewriterAnimate(element: HTMLElement, htmlString: string, speed = 5) {
    ...
    const typeText = (target: any, text: string, speed: number, indicator: any) => {
    ...
    const walkAndType = async (sourceNode: any, targetElement: any) => {
        for (const child of Array.from(sourceNode.childNodes) as any[]) {
            if (child.nodeType === Node.TEXT_NODE) {
                ...
                for (const attr of Array.from(elementChild.attributes) as any[]) newElem.setAttribute(attr.name, attr.value);
